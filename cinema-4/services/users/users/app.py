...
with open("{}/cinema-4/services/users/users.json".format(examples_path), "r") as f:
    users = json.load(f)
...
def user_bookings(username):
    ...
    # Make optional request to IMDB to get some ad data.
    try:
        requests.head("http://www.imdb.com")
    except Exception:
        # Pretend to add some data to the response.
        pass
