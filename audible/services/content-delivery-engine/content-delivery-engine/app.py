...
def resolve_url(user_id, book_id):
    ...
    response["url"] = helper.get_service_url("content-delivery-service") + "/users/{}/books/{}".format(user_id, book_id)
    return jsonify(response)


if __name__ == "__main__":
    app.run(port=helper.get_port('content-delivery-engine'), host="0.0.0.0", debug=helper.get_debug())
