...
def auth_and_log(user_id, book_id):
    ...
    try:
        ownership_response = requests.get("{}/users/{}/books/{}".format(helper.get_service_url(
            "ownership"), user_id, book_id), timeout=helper.get_timeout("ownership"))
    ...
    try:
        activation_response = requests.get("{}/books/{}".format(helper.get_service_url(
            "activation"), book_id), timeout=helper.get_timeout("activation"))
    ...
    try:
        stats_response = requests.post("{}/users/{}/books/{}".format(helper.get_service_url(
            "stats"), user_id, book_id), timeout=helper.get_timeout("stats"))
...
if __name__ == "__main__":
    app.run(port=helper.get_port('audible-download-service'),
